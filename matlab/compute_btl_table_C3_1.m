...
% P.1812-6 (using tl_p1812.m) for those path profiles in the table C3_1
...
%         
        ...
        for it = 1:length(t)
            ...
            if (Tpc>50 || Tpc < 1 || struct.f > 6.0 || struct.f < 0.03)
                warning(['Percentage and/or frequency are outside the valid range [1, 50]%, [0.03, 6] GHz, respectively. Skipping station number: ' num2str(stano{i}.number)])
                ...
                press = 1013;
                temp = 20;
                % vector of representative clutter heights
                R = 0*ones(size(struct.d));
                % vector of representative cluter types (all set to open/rural)
                Ct = 2*ones(size(struct.d));
                
                Lb = tl_p1812(struct.f, Tpc, struct.d, struct.h, R, Ct, z, Htg, Hrg, FlagVP, pdr, 'lam_t', Phite, 'phi_t', Phitn, 'lam_r', Phire, 'phi_r', Phirn, 'Gt', Gtx, 'Gr', Grx);
...
filename_out = 'Results_Table_C3_1_P1812.xls';

fprintf(1,'%10s  %10s  %20s  %20s  %20s  %20s  %20s\n','Stat. no.', 't (%)', 'Measured PL (dB)', 'P.2001 (dB)', 'PDR (dB)', 'PE P.2001 (dB)', 'PE PDR (dB)');

A = {'Stat. no.', 't (%)', 'Meas. BTL (dB)', 'P.1812 (dB)', 'PDR (dB)', 'PE P.1812 (dB)', 'PE PDR (dB)'};
...
legend('P.1812-6', lstr1, 'PDR P.1812-6', lstr2)
xlabel('PE (dB)')
ylabel('n.u.')
grid on
...
legend('PDR P.1812-6', 'P.1812-6','measurements', 'Location','southeast')
grid on
